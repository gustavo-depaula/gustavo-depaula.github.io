function makeBGLinks() {
  ...
  var searchNode = function (node, inserted_nodes) {
    ...
    if (verse_match == null) {
      ...
      while ((matched = book_chap_regex.exec(verse_match[0]))) {
        ...
        newLinkNode.href =
          bgHost + "/passage/?search=" + passage + "&version=" + that.version + "&src=tools"
  ...
  var getTooltip = function (reference, link) {
    ...
    remote_passage.id =
      "bg_remote_passage_script-" + reference.replace(/(?:%20)|\+|[^\x00-\x80]/g, "")
...
document.addEventListener("nav", () => {
  ...
  fetch("https://liturgiadiaria.site/")
    ...
    .then((data) => {
      ...
      document.getElementById("data").textContent = `${data.liturgia}, ${data.data}`

      const { mainText: antifonaEntrada, reference: antifonaEntradaSalmo } = extractReference(
        data.antifonas.entrada,
      )
      document.getElementById("introit-antiphon").textContent = antifonaEntrada
      document.getElementById("introit-reference").textContent = antifonaEntradaSalmo

      document.getElementById("first-reading-title").textContent = data.primeiraLeitura.titulo
      document.getElementById("first-reading-reference").textContent =
        `(${data.primeiraLeitura.referencia})`

      document.getElementById("first-reading").innerText = addNewlineAfterPeriodExceptInQuotes(
